...
//    	
//    }
    public void add(String elem) { add(root, null, false, elem); }
    
    private boolean add (TNode t,TNode parent,boolean right,String d) {
    	if(t==null) { TNode n= new TNode();
    	n.setData(d);
    	n.setLeft(null);
    	n.setRight(null);
    	if(root==t) {root= n; return true;}
    	if(right) parent.setRight(n);
    	else parent.setLeft(n);
    	return true; } 
    	else if(d.compareTo(t.getData())<0)  { add(t.getLeft(),t,false,d); }
    	else{ add(t.getRight(),t,true,d); }
    	return true; 
    }
    ...
    private boolean contains(TNode t, String p) {
    	...
    	if(p.compareTo(t.getData()) == 0) return true; 
    	if(p.compareTo(t.getData()) < 0) return contains(t.getLeft(), p);
